...
void print_line_info(t_line *line)
{
	printf("Coordenadas: (%d, %d) -> (%d, %d)\n", line->x0, line->y0, line->xf, line->yf);

	if (line->xf - line->x0 != 0)
	{
		double slope = (double)(line->yf - line->y0) / (line->xf - line->x0);
		printf("Pendiente: %.2f\n", slope);
	}
	else
	{
		printf("La línea es vertical, no se puede calcular la pendiente.\n");
	}
}
